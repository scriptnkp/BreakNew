...
(function () {
  ...
  function filterEvents() {
    var rows = byViewCcy(EVENTS);

    if (F.imp === "high") rows = rows.filter(function (e) { return e._imp === "high"; });
    else if (F.imp === "medium") rows = rows.filter(function (e) { return e._imp === "medium"; });
    else if (F.imp === "low") rows = rows.filter(function (e) { return e._imp === "low"; });
  ...
  function renderList(rows) {
    ...
    if (!rows.length) {
      $("list").innerHTML = '<div class="card empty">ไม่พบข่าวตามเงื่อนไข<br>' +
        '<span class="sm">ลองกด "ทั้งสัปดาห์" หรือ "ทุกแรง"</span></div>';
      return;
    }
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      html += '<article class="ev' + side + '">' +
        ...
        '<div class="ev-more">' +
          '<div class="exp"><h4>📌 ข่าวนี้คืออะไร</h4><p>' + esc(g.n) + " — " + esc(g.w) + "</p></div>" +
          '<div class="exp"><h4>🥇 ผลต่อราคาทอง</h4><p>' + esc(g.g) + "</p></div>" +
        "</div>" +
      "</article>";
    }
  ...
  function renderChips() {
    ...
    for (var i = 0; i < rows.length; i++) {
      if (c[rows[i]._imp] !== undefined) c[rows[i]._imp]++;
    }

    $("nHigh").textContent = c.high;
    $("nMed").textContent = c.medium;
    $("nLow").textContent = c.low;

    var chips = document.querySelectorAll(".chip");
    for (var j = 0; j < chips.length; j++) {
      var v = chips[j].getAttribute("data-imp");
      var on = (F.imp === v) || (F.imp === "highmed" && (v === "high" || v === "medium"));
      if (on) chips[j].classList.add("on");
      else chips[j].classList.remove("on");
    }
  }
  ...
  function renderSummary(rows) {
    ...
    if (score === null) {
      $("buyPct").textContent = "--";
      $("sellPct").textContent = "--";
      $("biasTxt").textContent = "ไม่มีตัวเลขให้คำนวณ";
      bar.style.width = "50%";
      return;
    }

    var b2 = toBuyPct(score);
    $("buyPct").textContent = "B " + b2 + "%";
    $("sellPct").textContent = "S " + (100 - b2) + "%";
    bar.style.width = b2 + "%";
    var tone = b2 > 60 ? "โน้มขึ้น" : b2 < 40 ? "โน้มลง" : "ไซด์เวย์";
  ...
  function renderNext() {
    ...
    var hrs = Math.round((p[0]._t - now) / 3600000);
  ...
  function afterLoad(via) {
    ...
    if (F.view === "today") {
      var t = new Date();
      var has = EVENTS.some(function (e) { return sameDay(e._d, t); });
      if (!has) {
        log('วันนี้ไม่มีข่าว → สลับไปโหมด "ถัดไป"');
        setSeg("view", "upcoming");
        return;
      }
    }
    render();
  ...
  $("chips").addEventListener("click", function (e) {
    ...
    if (!chip) return;
    var v = chip.getAttribute("data-imp");
    setSeg("imp", F.imp === v ? "all" : v);
  });
