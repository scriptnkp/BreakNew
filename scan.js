/* ===== Scan สินทรัพย์ 9 กลุ่ม — responsive table/card ===== */
(function () {
  ...
  var GRP = {
    aipicks:     { n: "AI คัดให้",          i: "✨" },
    th:          { n: "หุ้นไทย (SET)",      i: "🇹🇭", alt: "stocks" },
    us500:       { n: "S&P 500 / Nasdaq",   i: "🇺🇸" },
    global:      { n: "หุ้นต่างประเทศ",     i: "🌍",  alt: "stocks" },
    energy:      { n: "พลังงาน",            i: "⚡" },
    crypto:      { n: "คริปโตเคอร์เรนซี",   i: "₿" },
    commodities: { n: "สินค้าโภคภัณฑ์",     i: "🛢️" },
    currencies:  { n: "สกุลเงิน / Forex",   i: "💱" },
    aifunds:     { n: "กองทุน AI (ETF)",    i: "🤖" }
  };
  ...
  function sigHTML(r) {
    ...
    if (r.score !== undefined && r.score !== null)
      o.push('<span class="bdg b-ai">AI ' + Math.round(r.score) + "</span>");
    ...
    else if (r.rsi !== null && r.rsi <= 30) o.push('<span class="bdg b-os">OS</span>');
  ...
  function tableHTML(list, dp, capLabel, maxAbs) {
    ...
    if (!list.length)
      h += '<tr><td colspan="9" class="dim" style="text-align:center;padding:34px">ไม่พบข้อมูล — หากเพิ่งอัปโค้ด ให้รัน GitHub Actions ใหม่</td></tr>';
  ...
  function cardsHTML(list, dp, capLabel, maxAbs) {
    ...
    for (var i = 0; i < list.length; i++) {
      var r = list[i];
      var rk = i === 0 ? " rk1" : i === 1 ? " rk2" : i === 2 ? " rk3" : "";
      var why = (r.why && r.why.length) ? '<div class="mwhy">💡 ' + esc(r.why.join(" • ")) + "</div>" : "";
      ...
      h += '<article class="mc ' + rowClass(r.pct) + '">' +
        ...
        "</div>" +
        '<div class="mc-s">' + sigHTML(r) + "</div>" + why +
      "</article>";
    }
    return h + "</div>";
  }
  ...
  function block(pill, cls, list) {
    ...
    var stockish = ["th","us500","global","energy","aifunds","aipicks"].indexOf(S.grp) > -1;
  ...
  function render() {
    ...
    var up = 0, dn = 0;
    for (var i = 0; i < wp.length; i++) { if (wp[i].pct > 0) up++; else if (wp[i].pct < 0) dn++; }
    $("sUp").textContent = up;
    $("sDn").textContent = dn;
    ...
    if (S.grp === "aipicks") {
      ...
      html = block("✨ AI Top Picks", "p-ai", all.slice(0, 20));
    } else {
